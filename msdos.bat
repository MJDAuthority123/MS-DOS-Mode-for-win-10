...
if not exist %systemdrive%\dosexec.bat (
	...
	@echo echo Starting MS-DOS... >> %systemdrive%\dosexec.bat
	@echo pause >> %systemdrive%\dosexec.bat
	@echo cls >> %systemdrive%\dosexec.bat 
	@echo cd \ >> %systemdrive%\dosexec.bat
	@echo echo To restart Type "reboot"	to get back to windows type "win"	to turn off the computer type "turnoff" >> %systemdrive%\dosexec.bat
	echo.
	echo =-=-=-=-=-= DOSEXEC.BAT =-=-=-=-=-=
